...
#### 9/6/2025 Monday
- Added means of visualisation in eval script, cannot see environment but i think its not bad, can see when the agents are unsafe. 
- Created launch file to deploy mac_sim node in sim, need to test and see if it works.
- The more pertinent issue is to create more dangerous states, so as to greater drive CBF learning. 
- Working `mac-sim` in single agent setting! Took more work than expected. Generally works as expected. Going to try 2 agent setting and log flight data. Success.
- after the above, try to get new dataset of closer positions. to counteract the issue of DYNUS freezing or a unattainable goal being published (i.e. goal in tree/pillar), add a countdown to pub new goal if stuck for too long

#### 10/6/2025 Tuesday
- Wanted to gather more flight data yesterday but Docker was somehow stealing so much disk space. Fix obtained [here](https://medium.com/@piyushkashyap045/why-docker-keeps-eating-up-your-disk-space-and-how-to-fix-it-48d2ecf3dc65)
- Also had the bright idea of increasing number of agents to 5 so that we increase the probability of danger states
- Collected 6 sets of training data. Each has 6 agents flying in a 10x10x1 space. Spaces change across the sets. Sets either had 10 or 20 waypoints. Despite efforts to increase the frequency of danger states through a smaller flight area, more agents (twice) and more waypoints, a sanity check on one of the sets revealed about 0.95 safety rate, which is still a decrease but not so significant. On average, the number of scenes has also decreased by order 10, which should speed up traning.
- Decided to increase learning rate of CBF network parameters in particular, from 1e-4 to 5e-4. Hopefully that has some effect. Fairly confident that the port is correct. 
- Generally, I believe I have achieved my goals that I set out at the start. All that "remains" is the training and the results/effectiveness of it. Continue pushing it may or may not be worth it i.e. diminishing returns and would require more work than gain. My instinct is to say that its the lack of danger training scenarios. For context, when I tried the original MACBF, initial safety rates were like 0.72, which probably meant high loss values to effect greater changes. I think it is difficult to get that from DYNUS, because it is inherently a trajectory planner that tries to be safe, so I think it might be challenging to get it to put agents in precarious positions required for the training. We could swap out the planner, but then we would have to develop the sim (if it does not exist) for it and there is no guarantee that the same issue won't arise (planners generally want to be safe). OG MACBF seemed to create trajectories independently. Maybe that's the strat. But really troublesome to gather the data like that? Need to do agent by agent? Then concatenate the files. 
- Feel like I should have been able to done all this faster also. I guess its okay for 3 weeks of progress? Considering Bumblebee and lack of internet. 
- I also had to leave work after lunch and bring dev laptop home to do work because I had no internet. very sad :( came back to drop off laptop for training
- next two days help out at brainhack, friday meet prof for SAFMC, hopefully something to get started on! exciting. reservist next week so cannot check on training. maybe pop by saturday just to make sure things are going smoothly and that the learning rate change didnt screw us over.